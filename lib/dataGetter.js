...
DataGetter.prototype.serialize = function(data) {
  var lastUse = null;
  for (var i = 0; i < data.saved_macs.length; i++) {
    data.saved_macs[i].mac = this.util.uglifyMac(data.saved_macs[i].mac) // uglify the data just in case
...
DataGetter.prototype.getItemByName = function(name) {
  ...
  if (/\.|\[|\^|\$/.test(name)) {
      var regexp = true;
      try {
        "".match(name); // test if it is valid regexp
      } catch (e) {
          regexp = false;
      }
  }
  ...
  for (var index in list) {
    ...
    if (list[index].name == name || (regexp && list[index].name.match(name))) {
...
DataGetter.prototype.updateItemTime = function(name) {
  var i = this.indexOfDev(name);
  var now = Date.now();
  this.data.saved_macs[i].lastUse = now;
  return now;
};
