...
CommandExecuter.prototype._handleUp = function(cb) {
  ...
  for (var i = 1; i < this.argv._.length; i++) {
    ...
    if (device.length === 0) {
      if (!this.util.checkMac(data)) { // Is it even a mac?
        ...
        device = []
        device.push({mac:this.util.uglifyMac(data)});

      }
    }
    wakeDevices = _.uniqBy(wakeDevices.concat(device), "mac");
  }

  wakeDevices.forEach(function(device){
    var mac = device.mac;
    var successout;
    if(device.hasOwnProperty('name')){
      self.dg.updateItemTime(device.name);
      successout = "  Sending magic packet to ".green + device.name + "[" + self.util.beautifyMac(device.mac) + "]";
    } else {
      successout = "  Sending magic packet to ".green + mac;
    }
    self.output.stdOut(successout);
  ...
  //               doNextWake();
  //           }
  //       }, delay * 10); //one tenth of a second
  //     }
  //
  //
  //     //start first time
